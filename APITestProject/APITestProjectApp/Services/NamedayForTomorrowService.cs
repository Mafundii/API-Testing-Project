...
public class NamedayForTomorrowService
{
    #region Properties
    ...
    public NamedayForTomorrowService()
    {
    ...
    {
        Response = await CallManager.MakeRequest("tomorrow", parameters, method);
    ...
    {
        Response = await CallManager.MakeRequest("tomorrow", new Dictionary<string, string>(), method);
        JsonResponse = JObject.Parse(Response);
        NamedayTodayDTO.DeserialiseResponse(Response);
    }
