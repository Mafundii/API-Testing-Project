...
public class NamedayForDateService
{
    #region Properties
    ...
    {
        Response = await CallManager.MakeRequest("getdate", parameters, method);
    ...
    {
        Response = await CallManager.MakeRequest("getdate", new Dictionary<string, string>(), method);
        JsonResponse = JObject.Parse(Response);
        NamedayTodayDTO.DeserialiseResponse(Response);
    }
